...
class OpusManager : NSObject, OpusStreamHandler, AFSoundcardDelegate {
    ...
    fileprivate var _opus: Opus?                                    // Opus instance
    ...
    fileprivate var _rxInterleaved: [Float]!                        // output of Opus decoder
    fileprivate let _rxInterleavedPtr: UnsafeMutablePointer<Float>!
    ...
    fileprivate let _txInterleavedPtr: UnsafeMutablePointer<Float>!
...
//        print("txAudio - \(start ? "start" : "stop")")
    ...
    func inputReceived(from card: AFSoundcard!, buffers: UnsafeMutablePointer<UnsafeMutablePointer<Float>?>!, numberOfBuffers n: Int32, samples: Int32) {
        ...
        guard n == 2 else {
            _log.msg("Opus Tx, input not stereo", level: .error, function: #function, file: #file, line: #line)
            return
        }
        ...
        if result < 0 { _log.msg(String(cString: opus_strerror(result)), level: .error, function: #function, file: #file, line: #line) }
